...
grahamScan ps
    ...
    where
        sortedSet = map (\p -> transform (neg start) p) $ sortAngle $ map (\p -> transform start p) $ delete start ps
...
main = do
    ...
    let ps = map (\(x, y) -> Point x y) b
    let start = foldl1 safeEdge ps
    let sortedSet = map (\p -> transform (neg start) p) $ sortAngle $ map (\p -> transform start p) $ delete start ps
